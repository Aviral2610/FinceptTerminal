...
class AgnoTradingService {
  ...
  /**
   * Sync API keys to the Agno system
   */
  private async syncApiKeys(): Promise<void> {
    try {
      const apiKeys = await this.getApiKeysMap();
      if (Object.keys(apiKeys).length > 0) {
        await invoke('agno_set_api_keys', { apiKeysJson: JSON.stringify(apiKeys) });
      }
    } catch (error) {
      console.warn('[AgnoService] Failed to sync API keys:', error);
    }
  }

  /**
   * Parse response from Agno commands
   */
  private parseResponse<T>(response: unknown): AgnoResponse<T> {
    ...
    return response as AgnoResponse<T>;
  }
